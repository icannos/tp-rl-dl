...
import numpy as np
import copy
from agents.maddpg import maddpgAgent, maddpgTrainer
...
def plt_all_graphs(rewards):
    rewards = np.array(rewards).transpose()

    for agent in range(rewards.shape[0]):
        X = [i for i in range(rewards[agent].shape[0])]

        plt.plot(X, rewards[agent, :])
        plt.savefig(f"{game}-{agent}.png")

if __name__ == '__main__':
    env,scenario,world = make_env(game)

    agents = [maddpgAgent(i, env.observation_space[i].shape[0], env.action_space[i], len(env.agents)) for i,a in enumerate(env.agents)]
